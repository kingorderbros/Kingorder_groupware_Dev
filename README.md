...
목업(`../소스/Index_ver1.0.html`)에서 **샘플 데이터를 모두 빼고**, GitHub · Supabase · Cloudflare 로 돌아가게 꾸린 소스입니다.
...
├─ tools/
│   ├─ strip-sample-data.js   목업 → 개발환경 index.html 변환 (샘플 제거 · 저장소 치환)
...
화면 코드 자체는 목업과 같습니다. 목업에서 기능을 고치면 `npm run rebuild-from-mockup` 으로 다시 뽑을 수 있습니다
(단, 개발환경 `index.html` 을 직접 고쳤다면 그 변경을 먼저 목업에 반영해야 덮이지 않습니다 — 아래 "작업 흐름" 참고).
...
- 로그인 아이디는 **이메일**입니다 (`@company.com` 앞부분만 넣어도 됩니다). 목업의 데모 접두어 로그인(`sales` · `ops` …)은 없습니다.
...
- 목업(`../소스`)은 발표·시연용 샘플 데이터본으로 남겨 둡니다. 목업을 고친 것을 이쪽에 가져오려면
  `npm run rebuild-from-mockup` 을 돌리는데, 이 명령은 **개발환경 index.html 을 덮어씁니다** — 개발환경에만 있는 변경이 있으면 먼저 목업에 옮기세요.
  2026-09-16 부터 개발환경에만 있는 기능(로그인 비밀번호 등)이 있어 **스크립트가 스스로 멈춥니다** (`--force` 를 붙여야 덮어씀).
  (두 갈래를 오래 함께 가져가지는 마세요. 개발환경이 자리 잡았으니 목업은 보관용으로 두고 이쪽만 고치는 편이 안전합니다.)
