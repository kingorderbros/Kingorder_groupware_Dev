...
## 2026-09-23 — 구글 캘린더 연동 (4) 안 맞춰지던 것 고침

**무엇이 문제였나 (서버 기록으로 확인)**

1. **직원 구글 계정 주소가 23명 모두 비어 있었습니다.** 그래서 개인 캘린더가 하나도 만들어지지 않았고
   (`gcal_calendars` 9줄 = 공유 3 + 본부 6), 공유도 한 건도 나가지 않았습니다(`share_state` 전부 `none`).
   폰에 아무것도 안 뜨는 가장 큰 이유입니다.
2. **주고받기가 한 번도 돌지 않았습니다.** `gcal_links` 0줄 · `gcal_state` 0줄.
   `syncPush` 는 끝에 반드시 표식을 남기므로, 표식이 없다는 것은 한 번도 불리지 않았다는 뜻입니다.
   → Cloudflare 예약 실행(`[triggers]`)이 아직 안 걸려 있을 가능성이 큽니다. 대시보드에서 확인이 필요합니다.

**고친 것 — 예약 실행에만 기대지 않게**

- `/api/calendar/sync` 를 **로그인한 사람이면 부를 수 있게** 했습니다(전체를 훑는 일이라 사람별 자료가 아닙니다).
- **일정캘린더 화면을 보고 있는 동안 브라우저가 1분마다** 이것을 부릅니다. 서버 예약 실행이 꺼져 있어도
  누군가 그룹웨어를 열어 두면 맞춰집니다. 다른 탭을 보고 있거나 다른 화면이면 쉬어 갑니다.
- 여럿이 동시에 봐도 구글을 들볶지 않도록 **20초 안에 또 부르면 건너뜁니다**(`syncBoth(env, {minGapMs})`).
  관리자의 [지금 맞추기] 는 `force` 로 기다리지 않고 바로 돕니다.
...
## 2026-09-23 — 구글 캘린더 연동 (6) 직원용 안내 팝업

**일정캘린더 화면 오른쪽 위에 [휴대폰 연동 방법] 버튼**을 넣었습니다. 누르면 팝업이 열립니다.
(자리는 사용자 요청으로 **'전체 캘린더' 드롭다운 앞**, 그 줄의 맨 앞입니다.)

직원이 혼자 읽고 따라 할 수 있게 세 토막으로 나눴습니다.

1. **먼저 할 일(공통)** — 구글 계정 주소를 관리자에게 알려 주기 · 공유 메일의 링크 누르기.
   공유 메일 링크를 안 누르면 아무것도 안 되는데 그냥 지나치기 쉬워, 따로 떼어 맨 앞에 두었습니다.
2. **휴대폰에서 설정하기** — [안드로이드] · [아이폰] 을 눌러 자기 것만 봅니다.
   처음 열 때 쓰는 기기를 보고 알맞은 쪽을 먼저 보여 주고, 고른 것은 그 브라우저에 기억해 둡니다.
   - 안드로이드: 계정 확인 → 구글 캘린더 앱 ☰ 에서 '킹오더' 캘린더 체크
   - 아이폰: 계정 추가 → **`calendar.google.com/calendar/syncselect` 에서 체크**(가장 많이 빠뜨리는 단계라 눈에 띄게)
     → 아래로 당겨 새로고침
   - 양쪽 모두 **일정을 만들 때 캘린더를 '킹오더 …' 로 바꿀 것**을 굵게 적었습니다. 이걸 안 하면 본인
     개인 캘린더에 저장돼 그룹웨어로 오지 않습니다 — 실제로 시험하며 겪은 부분입니다.
3. **자주 겪는 일** — 폰 일정이 안 올 때 · 개인 일정이 회사에 보이는지 · 삭제가 양쪽에 퍼지는지 ·
   참석 여부 응답 · 공유 메일이 안 왔을 때.

아이폰의 15분 제약은 숨기지 않고 그대로 적고, 급하면 당겨 새로고침하거나 구글 캘린더 앱을 쓰라고 안내합니다.
폰 → 그룹웨어 방향은 1분이라 빠르다는 것도 함께 적었습니다.

- 새 DOM id: `phone-sync-modal` · `phone-sync-tab-android` · `phone-sync-tab-ios` · `phone-sync-android` · `phone-sync-ios`
- 새 함수: `openPhoneSyncHelp()` · `closePhoneSyncHelp()` · `setPhoneSyncOs()` · `phoneSyncGuessOs()`
- 고른 기기는 사람별 값이라 `localStorage`(`kobPhoneSyncOs`)에 둡니다.
- 검증: `npm run check` 전체 통과. 화면 눈 확인은 배포 뒤.

## 2026-09-23 — 연동 안내에 구글 캘린더 쓰는 법 추가 · 조직도에 표시하지 않기

**1) [휴대폰 연동 방법] 팝업에 3번 토막 추가 — '구글 캘린더 앱 쓰는 법'**

연결만 해 놓고 쓸 줄 몰라 묻는 일이 없도록, 앱에서 실제로 하는 일을 적었습니다.
볼 캘린더 고르기 · 일정 만들기 · 고치기/지우기 · 참석 여부 답하기 · 알림 · 캘린더 색 · PC 웹.

일정 만들 때 **저장 전에 캘린더를 '킹오더 …' 로 바꿔야 한다**는 것을 여기서도 다시 적었습니다.
연동에서 가장 많이 어긋나는 곳이라 설정 안내와 사용 안내 양쪽에 두었습니다.
아이폰 쓰는 분께는 구글 캘린더 앱을 받으면 15분이 1분으로 줄어든다는 것도 함께 적었습니다.

버튼 자리는 **'전체 캘린더' 드롭다운 앞**(머리 줄 맨 앞)으로 옮겼습니다.

**2) 구성원 창에 [조직도에 표시하지 않음]**

계정은 만들되 조직도에는 내보내지 않는 경우가 있어 체크 한 칸을 넣었습니다.

| 켜면 | 나오는 곳 |
| --- | --- |
| 조직도(계층도 · 카드형 · 본부별 목록 · 검색) | **안 나옴** |
| 사용자/권한관리 목록 | 나옴 (`조직도 제외` 라고 회색 표시) |
| 담당자 · 참석자 · 카드 소유자 고르기 | 나옴 |
| 로그인 · 권한 | 그대로 |

- 저장 항목은 `gwUsers.v1` 의 `hideInOrg`. 화면은 `isOrgHidden()` · `orgChartMembers()` 에서 거릅니다.
  관리자 계정을 조직도에서 빼던 방식(`isOrgAdminAccount`)과 같은 자리에 붙여, 조직도만 영향을 받습니다.
- **본부를 지울 수 있는지 볼 때는 숨긴 사람도 셉니다**(`orgMembers()` 를 그대로 씀).
  숨겼다고 해서 그 본부가 빈 것으로 보여 실수로 지워지는 일을 막기 위해서입니다.
- 새 DOM id: `org-edit-hide-org`
- 검증: `npm run check` 전체 통과. 화면 눈 확인은 배포 뒤.
